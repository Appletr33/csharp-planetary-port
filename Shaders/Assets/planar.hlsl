...
#else
    const auto height = sample_height(tile).x;
...
void fragment(input: FragmentInput) -> FragmentOutput {
    ...
    auto normal = sample_normal(tile, info.world_normal);

    if (info.blend.ratio > 0.0) {
        const auto tile2 = lookup_tile(info.coordinate, info.blend, 1u);
